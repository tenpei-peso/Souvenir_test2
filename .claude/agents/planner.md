...
- 類似の実装をレビューする
- 再利用可能なパターンを検討する
- 既存のコンポーネント、ユーティリティ、API、画面パターンを棚卸しする
- 調査範囲は、まず同一アプリ内、次に共有ライブラリ、最後に同一モノレポ内の近接機能とする
- 既存資産がそのまま使えるか、一部修正で使えるか、使用意図や用途が同じかを判定する
- 上記の条件を満たす既存資産がある場合は、採用方針または不採用理由を計画に必ず記載する
- 条件に当てはまる候補が見つからない場合のみ、該当なしとして次に進む
...
- [候補1: そのまま使える / 一部修正で使える / 使用意図や用途が同じ / 該当なし]
- [採用方針または不採用理由]
- [調査範囲: 同一アプリ / 共有ライブラリ / 同一モノレポ内の近接機能]
...
4. **既存資産の再利用を判定する**: 既存の形をそのまま使える、一部修正だけで使える、使用意図や用途が同じ既存資産がある場合は、再利用の検討と記載を必須にする
5. **無理な流用は避ける**: 責務不一致、過剰依存、影響範囲の増大、設計の歪みが出る場合は再利用を見送り、その理由を明記する
6. **パターンを維持**: 既存のプロジェクト規約に従う
7. **テストを可能に**: 変更を簡単にテストできるように構造化する
8. **段階的に考える**: 各ステップが検証可能であるべき
9. **決定を文書化**: 何をするかだけでなく、なぜそうするかを説明する
