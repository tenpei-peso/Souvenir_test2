...
description: PostgreSQL の SQL、マイグレーション、スキーマ、インデックス、RLS、クエリ性能調査を扱うときに使う。一般的なバックエンド実装や広いセキュリティ監査には使わない。
paths:
  - "**/*.sql"
  - "supabase/**"
  - "database/**"
...
- コネクションプーリングの設定時
- 一般的な API 実装ではなく、PostgreSQL 自体の判断が必要なとき
...
_[Supabase Agent Skills](Supabase Agent Skills (credit: Supabase team))（MITライセンス）に基づく_
